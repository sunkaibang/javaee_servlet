...
<html>
  <head>
    ...
    <title>My JSP 'publishQuery.jsp' starting page</title>
  ...
  <body>
    	...
    	<c:forEach items="${requestScope.request_pb.beanList}" var="cstm">
			<tr>
    			...
    			if(request.getSession().getAttribute("session_manager")!=null){
    			 %>
				<td>
				    <a href="<c:url value='updateData?method=modifyHouseFirst&id=${cstm.id }'/>">编辑</a>
				</td>
				<td>
					<a href="<c:url value='updateData?method=deleteHData&id=${cstm.id }'/>">删除</a>
				</td>
				<%
    			} %>
			</tr>
   ...
   <br/>
    <a href="javascript:toExecl('tbid','<c:url value='/toExcelServlet'/>','出版社信息表')" >导出表格</a>
  </body>
  </center>
</html>
