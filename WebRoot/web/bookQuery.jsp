...
<html>
  ...
  <center>
    <table border="1" id="tbid">
    	...
    	<c:forEach items="${requestScope.request_pb.beanList}" var="cstm">
			<tr>
				...
				<td>${cstm.name}</td>
				<td>${cstm.classId}</td>
				<td>${cstm.publishHouseId}</td>
				<td>${cstm.date}</td>
				<td>${cstm.editNumber}</td>
				<td>${cstm.introduce}</td>
				<td>${cstm.state}</td>
				<%
    			if(request.getSession().getAttribute("session_manager")!=null){
    			 %>
				<td>
				    <a href="<c:url value='updateData?method=modifyBookFirst&uuid=${cstm.uuid }'/>">编辑</a>
				</td>
				<td>
					<a href="<c:url value='updateData?method=deleteBook&uuid=${cstm.uuid }'/>">删除</a>
				</td>
				<%
    			} %>
			</tr>
    ...
    <br/>
    <a href="javascript:toExecl('tbid','<c:url value='/toExcelServlet'/>','期刊信息表')" >导出表格</a>
    </center>
  </body>
</html>
